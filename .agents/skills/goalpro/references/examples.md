...
合格输出要把“自动发布器”拆成安全、合规、可验收的产品目标，而不是直接承诺绕过平台限制自动发帖：
...
Execution policy:
先做 inventory，确认是否已有账号模型、内容模型、队列系统和 UI 框架。若没有官方发布能力证据，默认交付“发布辅助器”而不是“全自动发帖器”：实现草稿、排期、检查、发布清单、人工确认和状态记录。只有在官方能力与用户授权都明确后，才接入真实发布动作。
...
4. 实现发布前检查和失败原因展示。
5. 实现人工确认发布路径；真实自动发布能力留在明确授权后的独立步骤。
...
Previous result to inspect:
上一轮最终报告、本地 URL/截图、内容状态表、模拟内容数据、发布前检查输出、失败日志、平台能力核对结论、用户对自动化程度的反馈。
...
Verification delta:
补充上一轮缺失的状态流转、截图、失败样例、授权边界或发布记录验证，并说明本轮新增证据关闭了哪些差距。
...
Next LOOP packet:
包含原始小红书发布辅助器目标、当前轮次、已关闭证据、开放差距、时间参数、下一轮焦点、要读取的状态表/截图/日志/平台能力证据、验证 delta 和暂停条件。
```
